...
class SendOtpTest extends TestCase
{
    ...
    {
        $expected = new SendOtp('test', '1000');
        $actual = SendOtp::createFromArray(['merchantId' => 'test', 'merchantOrderId' => '1000']);
    ...
    {
        $sendOtp = new SendOtp('test', '1000');
        $this->assertEquals('test', $sendOtp->getMerchantId());

        $sendOtp = new SendOtp('other', '1000');
    ...
    public function testGetMerchantOrderId(): void
    {
        ...
        $sendOtp = new SendOtp('other', '2000');
    ...
    {
        $sendOtp = new SendOtp('test', '1000');
        $expected = ['merchantId' => 'test', 'merchantOrderId' => '1000'];
        $this->assertEquals($expected, $sendOtp->toArray());

        $sendOtp = new SendOtp('shop', '2000');
        $expected = ['merchantId' => 'shop', 'merchantOrderId' => '2000'];
        $this->assertEquals($expected, $sendOtp->toArray());
    }
}
